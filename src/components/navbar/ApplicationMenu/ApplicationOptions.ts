...
// PE 1/3 - delete?
export const getCurrentApplicationByPath = (
  path: string
): IApplicationOption => {
  if (path.startsWith(PATHS.monerate.index)) {
    return applicationOptions.find(
      (option) => option.applicationName === "Monerate"
    );
  } else if (path.startsWith(PATHS.relearn.index)) {
    return applicationOptions.find(
      (option) => option.applicationName === "Relearn"
    );
  } else if (path.startsWith(PATHS.skillbase.index)) {
    return applicationOptions.find(
      (option) => option.applicationName === "Skillbase"
    );
  } else {
    return applicationOptions.find(
      (option) => option.applicationName === "Home"
    );
  }
};
